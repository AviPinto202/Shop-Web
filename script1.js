...
let imageArray = document.getElementsByClassName("productImages");
...
for (let i = 0; i < imageArray.length; i++) {
    ...
    function (event){
        ...
        imageArray[event.target.dataset.next].style.display = "block";
      }
    )
}

for (let i = 0; i < imageArray.length; i++) {
    imageArray[3].addEventListener(
        'click',
        function (event){
            imageArray[i].style.display = "";
            imageArray[event.target.dataset.next].style.display = "";
        }
    )
}
